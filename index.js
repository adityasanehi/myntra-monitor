...
function ScrapJDLaunch() {
    ...
    request(options, async (error, response, html) => {
        ...
        if (!error && response.statusCode == 200) {
            ...
            //Scrape title
            $('div.product-productMetaInfo').find('h4').each((i, el) => {
                productNames[i] = $(el).text().toUpperCase();
            });

            //Scrape link
            $('li.product-base').find('a').each((i, el) => {
                links[i] = "https://www.myntra.com/" + $(el).attr('href');
            });

            //Scrape picture
            $('picture.img-responsive').find('source').each((i, el) => {
